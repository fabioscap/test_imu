...
namespace test_imu {
  ...
  void ImuPreintegratorUKF::preintegrate_(const ImuMeasurement& m, Scalar dt) {
    ...
    input_noise_diag_ << gyro_noise_ / dt, acc_noise_ / dt;
    bias_noise_diag_ << bias_gyro_noise_ * dt, bias_acc_noise_ * dt;
  ...
  void ImuPreintegratorUKFSlim::preintegrate_(const ImuMeasurement& m, Scalar dt) {
    ...
    delta_incr_.get<3>().data() = (m.acceleration.cast<Scalar>());
    delta_incr_.get<4>().data() = (m.angular_vel.cast<Scalar>());
